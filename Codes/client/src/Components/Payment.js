...
import { clearCart, payCart } from '../actions/userActions';
...
class Payment extends Component {
    ...
    payCart=()=>{
        ...
        if(this.state.account_no.length>0 && this.state.exp_m.length>0 && this.state.exp_d.length>0 && this.state.account_name.length>0 && this.state.cvc.length>0){
            this.props.payCart(this.state).then(res=>{
            ...
            <Modal className="checkout-payment" isOpen={this.props.isOpen} toggle={this.props.toggle} >
                <ModalBody className="checkout" style={{borderTop: '4px solid #298dff'}} >
                    ...
                    <div className="item" style={{marginBottom: '16px'}}>
                        ...
                        <p className="item-description" style={{fontSize: '14px', color: 'grey', marginBottom: '0'}} >{this.props.user.name}</p>
                    </div>

                    <div className="item" style={{marginBottom: '16px'}}>
                        <p className="item-name" style={{fontWeight: 'bold', marginBottom: '0'}} >Billing Address</p>
                        <p className="item-description" style={{fontSize: '14px', color: 'grey', marginBottom: '0'}} >{this.props.user.address}</p>
                    </div>
                ...
                <ModalBody className="payment" >
                    ...
                    <form className="form-row">
                        <div className="col-sm-7">
                            <div className="form-group"><label htmlFor="card-holder">Card Holder</label><input required className="form-control" type="text" placeholder={this.props.user.name} name="account_name" onChange={this.onChange} /></div>
                        </div>
...
export default withRouter(connect(mapStateToProps, {clearCart, payCart})(Payment));
